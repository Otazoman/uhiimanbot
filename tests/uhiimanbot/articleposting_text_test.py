...
class TestArticlePostingText(TestCase):
    def setUp(self):
        self.mode = "test"
    ...
    def test_interval_post(self):
        ...
        with open(self.database_filepath, mode="r") as f:
            database = f.read()
            poststat = eval(database)[0]["poststatus"]
    ...
    def test_contents_summary_post(self):
        ...
        with open(self.database_filepath, mode="r") as f:
            database = f.read()
            poststat = eval(database)[1]["poststatus"]
        ...
        self.articleposting.summary_post(
            contents, articlepost_test_data.rss_template, title
        )
        with open(articlepost_test_data.output_summary_filepath, mode="r") as f:
            result = f.readlines()
